<template>
  <nav>
    <h1>{{ title }}</h1>
    <!-- 회원가입 화면에서만 로그인 페이지로 이동 -->
    <router-link v-if="currentPath === '/signup'" to="/" class="btn-close">
      <Icon icon="material-symbols:close" width="24" style="color: #1e1e1e" />
    </router-link>

    <!-- 다른 화면에서는 job-list로 이동 -->
    <router-link
      v-if="currentPath === '/' || currentPath === '/job-post' || currentPath.startsWith('job-post-update')"
      to="/job-list"
      class="btn-close"
    >
      <Icon icon="material-symbols:close" width="24" style="color: #1e1e1e" />
    ...
    <router-link v-if="currentPath.startsWith('/job-detail') || currentPath === '/user-profile'" to="/job-list" class="btn-close">
    ...
    <div class="right-icons" v-if="currentPath === '/job-list'">
      <router-link to="/user-profile">
        <Icon icon="teenyicons:user-circle-solid" width="24" style="color: #1e1e1e" />
      </router-link>
